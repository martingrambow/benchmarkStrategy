...
SUT_IP="$(gcloud compute instances describe $instanceName --zone='europe-west3-c' --format='get(networkInterfaces[0].accessConfigs[0].natIP)')"
...
gcloud compute scp -r $instanceName:~/profiles $resultPath/profiles --zone europe-west3-c 
